...
interface Inscripcion { alumno: Alumno; curso: Curso; }
...
export class Inscripciones {
  ...
  agregarInscripcion() {
    ...
    const yaExiste = this.inscripciones.some(
      i => i.alumno.id === this.alumnoSeleccionado!.id &&
           i.curso.id === this.cursoSeleccionado!.id
    );
    if (yaExiste) { alert('Esta inscripción ya existe'); return; }
    this.inscripciones.push({ alumno: this.alumnoSeleccionado, curso: this.cursoSeleccionado });
